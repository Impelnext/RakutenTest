...
export default class BirthdayOthers extends PubsubExtentions {
  @api itemKey;
  @api yearUnit;
  @api monthUnit;
  @api dayUnit;
  ...
  onRecivedMessage(that, message){
      ...
      if(message.componentName == 'RelationshipWithContractor'){
        if(message.componentValue == 'others1' || message.componentValue == 'others2' || message.componentValue == 'others3'){
          that.dispValue = true;
          that.othBirthday.value = '';
          that.othBirthday.style = "border: 1px solid #ccc;    border-radius: 4px;    background-color: #ffdddd;    padding: 8px 16px;    width: 100%;    appearance: none;    -webkit-appearance: none;";
        }else {
  ...
  renderedCallback() {
    ...
    this.othBirthday = document.getElementById('birthdayOthers');
    if(this.othBirthday.value == ''){
      console.log('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
      this.othBirthday.style = "border: 1px solid #ccc;    border-radius: 4px;    background-color: #ffdddd;    padding: 8px 16px;    width: 100%;    appearance: none;    -webkit-appearance: none;";
    }
  }
  ...
  createInputElementForJqueryUI(_this, appendDiv){
    if(appendDiv){
      ...
      this.appendInput.id="birthdayOthers"
      this.appendInput.name="nmBirthdayOthers"
