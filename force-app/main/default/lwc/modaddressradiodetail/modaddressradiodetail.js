...
export default class Modaddressradiodetail extends PubsubExtentions {
  @api itemkeyCheckBox;
  @api itemGuideMessage;
  @api majorItemName
  @api itemName1
  @api itemName2
  @api itemKeyPostCode;
  ...
  isChanged = false;
  confirmValue = '';
  labelDefaultConfirmAddress = "転居先住所（ご変更先住所）"
  ...
  renderedCallback() {
    ...
    if(this.confirmPhase) this.modaddress = null;
    if(!this.modaddress) {
  ...
  get dispModAddress(){
    return this.valueRadio == "変更がある / 登録している住所がわからない";
  }
  get styleModAddress(){
  ...
  get dispLabelConfirmAddress() {
    return this.labelConfirmAddress ? this.labelConfirmAddress : this.labelDefaultConfirmAddress
  }
  ...
  createInputElementForJqueryUI(_this, appendDiv){
    if(appendDiv){
      if(!this.appendInput){
        ...
        if(this.htmlValueMoveDay) this.appendInput.value = this.htmlValueMoveDay;
      }
