...
export default class Birthday extends PubsubExtentions {
  ...
  onRecivedMessage(that, message){
      ...
      if(message.componentName == 'RelationshipWithContractor'){
        if(message.componentValue == 'others1' || message.componentValue == 'others2' || message.componentValue == 'others3'){
          that.dispValue = true;
          that.othConBirthday.value = '';
          that.othConBirthday.style = "border: 1px solid #ccc;    border-radius: 4px;    background-color: #ffdddd;    padding: 8px 16px;    width: 100%;    appearance: none;    -webkit-appearance: none;";
        }else {
  ...
  renderedCallback() {
    ...
    this.othConBirthday = document.getElementById('birthdayOtherContractor');
    if(this.othConBirthday.value == ''){
      this.othConBirthday.style = "border: 1px solid #ccc;    border-radius: 4px;    background-color: #ffdddd;    padding: 8px 16px;    width: 100%;    appearance: none;    -webkit-appearance: none;";
    }
  }
  ...
  createInputElementForJqueryUI(_this, appendDiv){
    if(appendDiv){
      ...
      this.appendInput.id="birthdayOtherContractor"
      ...
      this.appendInput.onchange =function(e){_this.changeValueHandler3(e);};
