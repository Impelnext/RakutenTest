({
    ...
    getSingleByteDecimalCheck: function(component,event,inputValue,resultValue) {
    	
        var getValue = component.get(inputValue);
        var decimalRegx = /^\d{0,18}(?:\.\d)?$/;
        if (getValue === undefined || 
            getValue === null || 
            getValue === '' || 
            decimalRegx.test(getValue.trim())) {
            if (getValue !== undefined &&  
                getValue !== null) {
                component.set(inputValue, getValue.trim());
            }
            component.set(resultValue, false);
        }
        else {
