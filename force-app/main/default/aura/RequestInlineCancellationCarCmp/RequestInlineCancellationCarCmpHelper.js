({
    ...
    getinterruptionCertificateReqPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'InterruptionCertificateRequirement__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var interruptionCertificateReqPickMap = [];
                for(var key in result){
                    interruptionCertificateReqPickMap.push({key: key, value: result[key]});
                }
                component.set("v.interruptionCertificateReqPickMap", interruptionCertificateReqPickMap);
            }
        });
        $A.enqueueAction(action);
    },
