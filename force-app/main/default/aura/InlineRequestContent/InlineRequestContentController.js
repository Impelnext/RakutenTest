({
    ...
    onRequestContentChange : function(component, event, helper){
        ...
        if (ReqConType=="差し戻し回答依頼"){
        	...
        	component.find("ReturnName").set("v.disabled",true); 
            component.find("ReturnContact").set("v.disabled",true);
            component.find("ReturnName").set("v.value",''); 
        	...
        	component.find("CallBackStartTime").set("v.disabled",false);
            component.find("CallBackEndTime").set("v.disabled",false);
            component.find("ReturnName").set("v.disabled",false);
    ...
    setParentCmpValues:function(component, event, helper){
           	console.log("Entering onchange func");
        	...
        	console.log("obj.DetailsOfRequest__c"+obj.DetailsOfRequest__c);
        	 ...
        	 else if (FieldName=="ReturnName"){
        		obj.ReturnName__c=EventSource.get("v.value");
        		}
        	  else if (FieldName=="ReturnContact"){
        		obj.ReturnContact__c=EventSource.get("v.value");
        		}
        		console.log("obj in request content"+JSON.stringify(obj));
    },
    //Event handler
    saveReqContent : function (component, event, helper){
        var params = event.getParam('arguments');
       	var obj= params.parentNewRequestFromInqTyp;
        console.log("Entering method call"); 
        obj.SeihoRequestContents__c = component.get("v.innerCmpNewRequest").SeihoRequestContents__c;
        obj.DetailsOfRequest__c = component.get("v.innerCmpNewRequest").DetailsOfRequest__c;
        obj.EmergencyFlag__c = component.get("v.innerCmpNewRequest").EmergencyFlag__c;
        obj.CallBackStartTime__c = component.get("v.innerCmpNewRequest").CallBackStartTime__c;
        obj.CallBackEndTime__c = component.get("v.innerCmpNewRequest").CallBackEndTime__c;
        obj.ReturnName__c = component.get("v.innerCmpNewRequest").ReturnName__c;
        obj.ReturnContact__c = component.get("v.innerCmpNewRequest").ReturnContact__c;
        }
})