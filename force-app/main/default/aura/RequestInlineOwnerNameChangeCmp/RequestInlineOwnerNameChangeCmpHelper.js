({
	...
	gettheRelatnWitTheCurrOwnerPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'TheRelationshipWithTheCurrentOwner__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var theRelatnWitTheCurrOwnerPickMap = [];
                for(var key in result){
                    theRelatnWitTheCurrOwnerPickMap.push({key: key, value: result[key]});
                }
                component.set("v.theRelatnWitTheCurrOwnerPickMap", theRelatnWitTheCurrOwnerPickMap);
            }
        });
