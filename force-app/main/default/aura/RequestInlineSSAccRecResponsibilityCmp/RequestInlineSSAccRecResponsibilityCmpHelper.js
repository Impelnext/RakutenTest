({
    ...
    getOtherPartnerPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'RelationshipWithTheOtherPartner__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var otherPartnerPickMap = [];
                for(var key in result){
                    otherPartnerPickMap.push({key: key, value: result[key]});
                }
                component.set("v.otherPartnerPickMap", otherPartnerPickMap);
            }
        });
        $A.enqueueAction(action);
    },
    getContractorAndReporterPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'RelationshipContractorReporter__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var contractorAndReporterPickMap = [];
                for(var key in result){
                    contractorAndReporterPickMap.push({key: key, value: result[key]});
                }
                component.set("v.contractorAndReporterPickMap", contractorAndReporterPickMap);
            }
        });
        $A.enqueueAction(action);
    },
