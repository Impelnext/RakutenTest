({
    ...
    getResendDocumentsPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'ResendDocuments__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var resendDocumentsPickMap = [];
                for(var key in result){
                    resendDocumentsPickMap.push({key: key, value: result[key]});
                }
                component.set("v.resendDocumentsPickMap", resendDocumentsPickMap);
            }
        });
        $A.enqueueAction(action);
    },
