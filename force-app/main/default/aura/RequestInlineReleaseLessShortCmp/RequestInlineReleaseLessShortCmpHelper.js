({
	...
	getcancellationTypePicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'CancellationType__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var cancellationTypePickMap = [];
                for(var key in result){
                    cancellationTypePickMap.push({key: key, value: result[key]});
                }
                component.set("v.cancellationTypePickMap", cancellationTypePickMap);
            }
        });
        $A.enqueueAction(action);
    },
    getMemoPicklist: function(component, event,dependantSubject) {
        var subDepAction = component.get("c.getDependentMap");
        subDepAction.setParams({
            objDetail : 'Request__c',
            contrfieldApiName : 'Subject__c',
            depfieldApiName : 'Memo__c'
        });
        subDepAction.setCallback(this, function(subDepResponse){   
            if(subDepResponse.getState() === "SUCCESS"){
                ...
                component.set("v.memoPickMap",depSubResponse);
                var depnedentFieldSubProcessingMap = component.get("v.memoPickMap");
                ...
                if(ListOfDependentFieldsSubProcessing.length > 0){
                    this.fetchDepValuesSub(component, ListOfDependentFieldsSubProcessing,"v.memoPickList");    
                }   
            }
    ...
    getpetDeathDateConfirmationPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'PetDeathDateConfirmation__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var petDeathDateConfirmationPickMap = [];
                for(var key in result){
                    petDeathDateConfirmationPickMap.push({key: key, value: result[key]});
                }
                component.set("v.petDeathDateConfirmationPickMap", petDeathDateConfirmationPickMap);
            }
        });
        $A.enqueueAction(action);
    },
    getrefundUponCancellationPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'RefundUponCancellation__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var refundUponCancellationPickMap = [];
                for(var key in result){
                    refundUponCancellationPickMap.push({key: key, value: result[key]});
                }
                component.set("v.refundUponCancellationPickMap", refundUponCancellationPickMap);
            }
        });
        $A.enqueueAction(action);
    },
