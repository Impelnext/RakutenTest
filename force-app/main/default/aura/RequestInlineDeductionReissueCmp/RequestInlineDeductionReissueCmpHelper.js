({
	...
	getIsHandMadePicklist : function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'IsHandMade__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var isHandMadePickMap = [];
                for(var key in result){
                    isHandMadePickMap.push({key: key, value: result[key]});
                }
                component.set("v.isHandMadePickMap", isHandMadePickMap);
            }
            else if(state === "ERROR"){
