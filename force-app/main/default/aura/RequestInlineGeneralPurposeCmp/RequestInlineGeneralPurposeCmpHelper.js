({
	...
	getSubjectPicklist: function(component, event) {
    	...
    	getSubjectDetails2Picklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'SubjectDetails2__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var subjectdetails2PickMap = [];
                for(var key in result){
                    subjectdetails2PickMap.push({key: key, value: result[key]});
                }
                component.set("v.subjectdetails2PickMap", subjectdetails2PickMap);
            }
        });
        $A.enqueueAction(action);
    },
...
getOtherOrderFlagPicklist: function(component, event) {
	...
	getDeliveryDocumentsPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'DeliveryDocuments__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var deliveryDocumentsPickMap = [];
                for(var key in result){
                    deliveryDocumentsPickMap.push({key: key, value: result[key]});
                }
                component.set("v.deliveryDocumentsPickMap", deliveryDocumentsPickMap);
            }
        });
        $A.enqueueAction(action);
    },
