({
    getPreviousPickList: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'Previous__c'
		});
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {                
                ...
                for(var key in result){
                    previousPickMap.push({key: key, value: result[key]});
                }
                component.set("v.previousPickMap", previousPickMap);
            }
        });
        $A.enqueueAction(action);
    },
	getLaterPickList: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'Later__c'
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {                
                ...
                var laterPickMap = [];
                for(var key in result){
                    laterPickMap.push({key: key, value: result[key]});
                }
                component.set("v.laterPickMap", laterPickMap);
            }
        });
        $A.enqueueAction(action);
    },
