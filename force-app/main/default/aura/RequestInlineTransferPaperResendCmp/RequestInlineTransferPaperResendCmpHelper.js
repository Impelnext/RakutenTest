({
	...
	getResendingDocumentsProceduresPicklist: function(component, event) {
        var action = component.get("c.getSinglePicklist");
		action.setParams({
			objectName : 'Request__c',
			fieldName : 'ResendingDocumentsProcedures__c'
		});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {                
                var result = response.getReturnValue();
                var resendingDocumentsProceduresPickMap = [];
                for(var key in result){
                    resendingDocumentsProceduresPickMap.push({key: key, value: result[key]});
                }
                component.set("v.resendingDocumentsProceduresPickMap", resendingDocumentsProceduresPickMap);
            }
        });
        $A.enqueueAction(action);
    },
    
    getRetransmissionReasonPicklist: function(component, event, dependantSubject) {
        var subDepAction = component.get("c.getDependentMap");
        subDepAction.setParams({
            objDetail : 'Request__c',
            contrfieldApiName : 'Subject__c',
            depfieldApiName : 'RetransmissionReason__c'
        });
        subDepAction.setCallback(this, function(subDepResponse){
            if(subDepResponse.getState() === "SUCCESS"){
                ...
                component.set("v.retransmissionReasonPickMap",depSubResponse);
                var depnedentFieldSubProcessingMap = component.get("v.retransmissionReasonPickMap");
                ...
                if(ListOfDependentFieldsSubProcessing.length > 0){
                    this.fetchDepValuesSub(component, ListOfDependentFieldsSubProcessing,"v.retransmissionReasonPicklist");
                }
            }
        });
